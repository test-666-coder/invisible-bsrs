...
| 模組 | 預設模型 | 說明 |
| --- | --- | --- |
| 語音轉逐字稿 | `faster-whisper` + `Systran/faster-whisper-medium` | 本地 ASR，比 tiny/small 更穩，使用 CTranslate2 與量化推論降低硬體壓力；若硬體不足可降回 Transformers small/tiny。 |
...
| 逐字稿與心情轉量表 | `OPENAI_BSRS_MODEL` | 預設 `gpt-5.6-sol`，只送去識別化逐字稿與語音情緒摘要，使用 JSON schema 輸出 `schema_version=1.1.0` 報告。 |

BSRS 評分的 system prompt 預設放在 `prompts/bsrs_system_prompt.txt`，可由 `.env` 的 `BSRS_SYSTEM_PROMPT_FILE` 指到其他檔案。
